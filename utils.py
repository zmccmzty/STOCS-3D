...
class OptimizationProblemEvaluation:
    """Defines a result of evaluating an optimization objective and 
    constraints at some state.
    
    Variables, objectives, and constraints can be referred to by
    a string like 'key' (a constant through the trajectory), 'key_time' 
    (a trajectory variable) or 'key_time_other_indices' (a trajectory
    variable with sub-indices).  The delimiter can be customized.

    OR, they can be referred to by a tuple. This is slightly faster
    since less string processing is needed.
    """
    total_objective: float = 0.0                                        # the sum of all objective components
    objectives: Dict[Any,float] = field(default_factory=dict)           # a dictionary of named objective components
    variables: Dict[Any,FloatOrVector] = field(default_factory=dict)    # all variable values
    variable_bounds: Dict[Any,Tuple[FloatOrVector,FloatOrVector]] = field(default_factory=dict)     # bounds on variables
    constraints: Dict[Any,FloatOrVector] = field(default_factory=dict)                              # all constraint values
    constraint_bounds: Dict[Any,Tuple[FloatOrVector,FloatOrVector]] = field(default_factory=dict)   # bounds on constraints
    objectives_by_time: List[Dict[Any,float]] = field(default_factory=list)                         # objective components by time
    variables_by_time: List[Dict,Any,FloatOrVector] = field(default_factory=list)                   # variable values by time
    constraints_by_time: List[Dict[Any,FloatOrVector]] = field(default_factory=list)                # constraint values by time
    time_index: int = 1                                                 # index of the time variable
    delimiter: str = '_'                                                # delimiter for splitting keys

    def split_time(self,key):
        if isinstance(key,str):
            key = key.split(self.delimiter)
            if len(key) <= self.time_index:
                raise ValueError()
            tkey = int(key[self.time_index])
            return tkey,self.delimiter.join(key[:self.time_index]+key[self.time_index+1:])
        elif isinstance(key,tuple):
            if len(key) <= self.time_index:
                raise ValueError()
            return key[self.time_index],key[:self.time_index]+key[self.time_index+1:]
        raise ValueError()

    def join_time(self,key,t):
        if isinstance(key,tuple):
            return key[:self.time_index] + (t,) + key[self.time_index:]
        elif isinstance(key,str):
            if self.time_index != 1 or self.delimiter in key:
                key = key.split(self.delimiter)
                key = key[:self.time_index] + (str(t),) + key[self.time_index:]
                return self.delimiter.join(key)
            else:
                return (key,t)
        raise ValueError()

    def regularize(self):
        """Fills out elements by time.  If keys are strings then delimiter is
        used to split the key to get a tuple of key elements. If time_index is
        given then this will look to the tuple of key elements to find the time
        index."""
        
        if len(self.objectives) >= 0:
            for k,v in self.objectives.items():
                try:
                    key,vname = self.split_time(k)
                except Exception:
                    continue
                while key >= len(self.objectives_by_time):
                    self.objectives_by_time.append({})
                self.objectives_by_time[key][vname] = v
        elif len(self.objectives_by_time) > 0:
            for t in range(len(self.objectives_by_time)):
                for k,v in self.objectives_by_time[t].items():
                    self.objectives[self.join_time(k,t)] = v   

        if len(self.objectives) == 0:
            self.objectives['objective'] = self.total_objective
        elif self.total_objective == 0.0:
            self.total_objective = sum(self.objectives.values())
        ...
        if len(self.variables) >= 0:
            for k,v in self.variables.items():
                try:
                    ...
                    continue
                while key >= len(self.variables_by_time):
                    self.variables_by_time.append({})
                self.variables_by_time[key][vname] = v
        elif len(self.variables_by_time) > 0:
            for t in range(len(self.variables_by_time)):
                for k,v in self.variables_by_time[t].items():
                    self.variables[self.join_time(k,t)] = v
        ...
        if len(self.constraints) >= 0:
            for k,v in self.constraints.items():
                try:
                    ...
                    continue
                while key >= len(self.constraints_by_time):
                    self.constraints_by_time.append({})
                self.constraints_by_time[key][vname] = v

    def variable_trajectory(self,key:str) -> np.ndarray:
        """Extracts a variable's trajectory from the optimization result."""
        if key in self.variables_by_time[0]:
            return np.array([self.variables_by_time[t][key] for t in range(len(self.variables_by_time))])
        else:
            return np.array([self.variables[key] for _ in range(len(self.variables_by_time))])
    
    def objective_trajectory(self,key:str) -> np.ndarray:
        """Extracts an objective's trajectory from the optimization result."""
        if key in self.objectives_by_time[0]:
            return np.array([self.objectives_by_time[t][key] for t in range(len(self.objectives_by_time))])
        else:
            return np.array([self.objectives[key] for _ in range(len(self.objectives_by_time))])
    
    def constraint_trajectory(self,key:str) -> np.ndarray:
        """Extracts a constraint's trajectory from the optimization result."""
        if key in self.constraints_by_time[0]:
            return np.array([self.constraints_by_time[t][key] for t in range(len(self.constraints_by_time))])
        else:
            return np.array([self.constraints[key] for _ in range(len(self.constraints_by_time))])
    
    def set_variable_trajectory(self,key:str,values:list):
        """Sets a variable's trajectory in the optimization result."""
        if len(self.variables_by_time) == 0:
            self.variables_by_time = [{} for _ in range(len(values))]
        else:
            assert len(self.variables_by_time) == len(values),"Invalid length of values"
        for t,v in zip(range(len(values)),values):
            self.variables_by_time[t][key] = v
            self.variables[self.join_time(key,t)] = v
    
    def set_objective_trajectory(self,key:str,values:List[float]):
        """Sets an objective's trajectory in the optimization result."""
        if len(self.objectives_by_time) == 0:
            self.objectives_by_time = [{} for _ in range(len(values))]
        else:
            assert len(self.objectives_by_time) == len(values),"Invalid length of values"
        for t,v in zip(range(len(values)),values):
            self.objectives_by_time[t][key] = v
            self.objectives[self.join_time(key,t)] = v
    
    def set_constraint_trajectory(self,key:str,values:list):
        """Sets a constraint's trajectory in the optimization result."""
        if len(self.constraints_by_time) == 0:
            self.constraints_by_time = [{} for _ in range(len(values))]
        else:
            assert len(self.constraints_by_time) == len(values),"Invalid length of values"
        for t,v in zip(range(len(values)),values):
            self.constraints_by_time[t][key] = v
            self.constraints[self.join_time(key,t)] = v
